...
const ProblemList = () => {
  ...
  const fetchProblems = async () => {
    ...
    try {
      const response = await fetch(`/api/problems?page=${page}&limit=6`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 mt-16">
      <div className="flex justify-between items-center mb-8">
        ...
        <Link
          to="/create-problem"
          className="btn-save box-border flex items-center"
        >
          <svg
      ...
      {problems.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-400 mb-6">
            Be the first to report a community problem
          </p>
            ...
            <div ref={loaderRef} className="text-center py-6">
              {loading && (
                <div className="flex items-center justify-center">
                  <div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 mr-2 border-gray-700"></div>
                </div>
              )}
            </div>
          )}
          {!hasMore && (
            <div className="text-center text-gray-500 py-4">No more problems to load</div>
          )}
        </>
      )}
    </div>

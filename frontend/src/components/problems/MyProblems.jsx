...
import { setUserProblems, startLoading, setError } from '../../store/slices/problemSlice';

const MyProblems = () => {
  const dispatch = useDispatch();
  const { userProblems, loading, error } = useSelector(state => state.problems);
  ...
  const deleteProblem = async (problemId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        dispatch(setUserProblems(userProblems.filter(problem => problem._id !== problemId)));
      } else {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 mt-16">
      ...
      {userProblems.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <h3 className="text-xl font-medium text-white mb-2">You haven't reported any problems yet</h3>
          <p className="text-gray-400 mb-6">Start by reporting a problem in your community</p>
          <Link to="/create-problem" className="btn-primary">
              ...
              <div key={problem._id} className="card group hover:border-primary-500/30 transition-all duration-300 animate-slide-up">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-400">
                      {problem.upvotes?.length || 0} upvotes
                    </span>
                  </div>
                </div>
